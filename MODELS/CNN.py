...
np.savetxt('CNN_label.csv',t,delimiter=',')
...
class TextCNN(nn.Module):
    ...
    def __init__(self):
        ...
        label_num = 2# 标签的个数
        filter_num = 200 # 卷积核的个数
        filter_sizes = [int(fsz) for fsz in '6,7,8'.split(',')]
        vocab_size = 24
        self.embedding_dim = 128
        self.embedding = nn.Embedding(vocab_size, self.embedding_dim)
        self.convs = nn.ModuleList(
            [nn.Conv2d(1, filter_num, (fsz, self.embedding_dim)) for fsz in filter_sizes])
...
def testcalc_metrics1(data_iter, net,epoch):
    ...
    data1.to_csv('CNN'+str(epoch)+'.csv')
