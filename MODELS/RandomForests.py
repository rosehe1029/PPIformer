...
np.savetxt('RF_label.csv',t,delimiter=',')
...
print("MCC: %f " %matthews_corrcoef(test_label,prediction))
y_score=rf.predict_proba(test_data) #descision_function()不可用
np.savetxt('RF.csv',y_score,delimiter=',')
fpr,tpr,threshold=roc_curve(test_label,y_score[:,1])
roc_auc=auc(fpr,tpr)
